...
function addNewRowEnajenantes() {
    var table = document.getElementById("Enajenantes");
    var rowCount = table.rows.length;
    var cellCount = table.rows[0].cells.length;
    var row = table.insertRow(rowCount);
    for (var i = 0; i < cellCount; i++) {
        ...
        if (i < cellCount - 1) {
            ...
            cell.innerHTML = '<input class="btn btn-danger" '
                + ' type="button" value="delete" onclick="deleteRowEnajenantes(this)" />';
        }
    }
}
...
/* This method will delete a row */
function deleteRowEnajenantes(ele) {
    var table = document.getElementById("Enajenantes");
    var rowCount = table.rows.length;
    if (rowCount <= 1) {
        alert("There is no row available to delete!");
